# -*- coding: utf-8 -*-
"""project2_yz3922.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1S39_4Ifq05pFYu63qXS8E4ZN6uPAeg2o
...
1.  use any modules beyond [Python Standard Library](https://docs.python.org/3/library/). (besides duckdb)
2.  run any operating system shell command (prepended by exclamation point) besides the setup codes we provided.
3.  use SQL magic (%SQL, %%SQL)
...
* "**Trump** shows in the New York Times **Maga**zine " is a match
...
SELECT id, text
FROM tweets 
WHERE LOWER(text) LIKE '%maga%' AND
...
SELECT extract('year' FROM strptime(create_time, '%a %b %d %H:%M:%S %z %Y')) as year, 
extract('month' FROM strptime(create_time, '%a %b %d %H:%M:%S %z %Y')) as month, 
count(*) as count
FROM tweets 
...
FROM
(SELECT src, dst, 
row_number() over (partition by src, dst) as ct
FROM
(SELECT twitter_username AS src,
regexp_extract(text, '@([0-9A-Za-z_]+)', 1) as dst
FROM tweets 
WHERE text LIKE '%@%' AND 
LENGTH(dst) >= 1) a
) b
...
# ssert len(result.columns)== 2, "The result should contain two columns: src, dst!"
result
...
We define the indegree and average number of likes to be high or low based on the rules below:
   ...
   1) If `indegree < avg(indegree of all users in the graph)` then indegree is said to be low for the user, else it is considered high. **A user is in the graph if it's either src or dst of an edge.**
...
(SELECT DISTINCT src as username 
FROM Graph)
UNION
SELECT DISTINCT dst
FROM Graph
...
with indegree as (
  (SELECT dst as users, count(*) as indegree
  FROM Graph
  GROUP BY dst 
  ORDER BY count(*) DESC)
  UNION
  ...
  FROM Graph
  WHERE src NOT IN (
  SELECT dst FROM Graph)
  ),

likes as (
  (SELECT twitter_username as users, 
  avg(like_num) as like_count
  FROM tweets
  GROUP BY twitter_username 
  ORDER BY like_count DESC)
  UNION
  SELECT DISTINCT dst as user, 0
  FROM Graph
  WHERE dst NOT IN (
  select twitter_username FROM tweets)
  ),
...
popular as (
  SELECT i.users as users, indegree, like_count, 
  CASE WHEN indegree < (SELECT avg(indegree) FROM indegree GROUP BY user) AND 
  like_count < (SELECT avg(like_num) FROM tweets) THEN 'unpopular'
  WHEN indegree >= (SELECT avg(indegree) FROM indegree GROUP BY user) AND 
  like_count >= (SELECT avg(like_num) FROM tweets) THEN 'popular' 
  ELSE 'unknown' END as popular_status
  FROM indegree i
  JOIN likes l
  ON i.users = l.users
  ),
...
mentioned as (
  SELECT count(*) as mentioned
  FROM tweets 
  WHERE twitter_username IN (
    SELECT users FROM popular WHERE popular_status = 'unpopular'
    ) AND 
    regexp_extract(text, '@([0-9A-Za-z_]+)', 1) IN (
      SELECT users FROM popular WHERE popular_status = 'popular'
      )
      ),
...
SELECT CAST(a/b AS FLOAT) AS unpopular_popular
FROM
(SELECT (SELECT CAST(mentioned AS FLOAT) FROM mentioned) as a, (SELECT CAST(total AS FLOAT) FROM ttl) as b)
"""
...
q6 = """
with base as
(SELECT g1.src as a, g2.src as b, g3.src as c
FROM Graph g1
JOIN Graph g2
ON g1.dst = g2.src
JOIN Graph g3
ON g2.dst = g3.src
WHERE g3.dst = g1.src AND 
g1.src != g2.src AND 
g2.src != g3.src AND
g3.src != g1.src)

SELECT count(*)/3 as no_of_triangles from base
"""
...
It is given by the formula: ![](https://www.geeksforgeeks.org/wp-content/ql-cache/quicklatex.com-aafd3a0d9f8bb8325cf2b41a4a839bbf_l3.svg). A node u without inbound nodes will have empty B_u, and a node u without outbound nodes won't contribute to the PageRank of any other nodes.
...
def page_rank(n_iter, db):
    ...
    create_pr = """
    DROP TABLE IF EXISTS initial_page;
    CREATE TABLE initial_page AS
    SELECT username,
    (SELECT 1/CAST(count(*) AS FLOAT) FROM users) as page_rank_score
    FROM users;

    DROP TABLE IF EXISTS ct_out;
    CREATE TABLE ct_out AS
    SELECT src, 1/CAST(count(*) AS FLOAT) as ct_out
    FROM Graph
    GROUP BY src

    """
    # for any runq, include the database db!
    runq(create_pr, db)

    for i in range(n_iter):
        # Make one iteration of page rank
        update_pr = """
        DROP TABLE IF EXISTS initial_pr;
        CREATE TABLE initial_pr AS
        SELECT u.username, 
        CASE WHEN pr IS NOT NULL THEN pr
        ELSE 0 END AS page_rank_score
        FROM users u
        LEFT JOIN
        (SELECT dst as username, sum(i.page_rank_score*ct_out) as pr
        FROM Graph a
        JOIN ct_out b
        ON a.src = b.src
        JOIN initial_page i
        ...
        DROP TABLE IF EXISTS initial_page;
        ALTER TABLE initial_pr RENAME TO initial_page;
    ...
    return runq("SELECT * FROM initial_page LIMIT 20", db)
...
SELECT u.username,
CASE WHEN pr IS NOT NULL THEN pr
ELSE 0 END AS page_rank_score, 1 as n_iter
FROM users u
LEFT JOIN 
(SELECT dst as username, sum(i.page_rank_score*b.ct_out) as pr
FROM Graph a
JOIN (
  SELECT src, 1/CAST(count(*) AS FLOAT) as ct_out
    FROM Graph
...
SELECT u.username,
CASE WHEN pr IS NOT NULL THEN pr
ELSE 0 END AS page_rank_score, c.n_iter
FROM iteration u
LEFT JOIN 
(SELECT dst as username, sum(i.page_rank_score*b.ct_out) as pr, 
n_iter+1 as n_iter
...
SELECT username, page_rank_score FROM iteration
WHERE n_iter = 20
ORDER BY page_rank_score DESC
LIMIT 20
...
Upload the "project2.py" (make sure the name is exactly the same name) to Gradescope project2. We have tests to check that your .py file is executable. Make sure you pass the first two tests.
 For the first test 'Check fields readable (0.0/0.0)', you should find 'All fields are readable!' in the end of output.
 For the second test 'Check submitted files (0.0/0.0)', you should find 'All required files submitted!' in the end of output.
 Failing these two tests will result in an automatic 0 for project2, and we **won't** accpet regrade request if you fail these two tests.
 If you see error messages from the gradescope, try to comment out the assert and runq functions causing these errors. 
 (HINT: we use a different test database for grading. If your codes run well in the given database but fail in gradescope, it means your solution is wrong) 
